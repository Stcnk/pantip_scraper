...
class Topic:
    def __init__(self, tid, name, author, author_id, story, likeCount, emoCount, emotions, tags, time, commentCount=0, comments=[]):
        ...
        self.story = clean_content(story)  # Clean the story content
...
class Comment:
    def __init__(self, num, user_id, user_name, replyCount, replies, message, emotions, likeCount, dateTime):
        ...
        self.message = clean_content(message)  # Clean the message content
        self.emotions = emotions
        self.likeCount = likeCount
    ...
    def get_comments_from_link(tid):
        ...
        while(index < 4):
            ...
            if (comment_response.reason == 'OK'):
                ...
                index = index + 1
            if index == 4:
                rData = ReturnData(False, "Cannot get comments %s: "%(tid) + comment_response.reason)
                return rData
    ...
    def convertPantip2Python(comment):
        ...
        if 'replies' in comment and comment['replies']:
            for reply in comment['replies']:
                replies.append(Comment.convertPantip2Python(reply))
        ...
        return Comment(
            ...
            replies, 
            clean_content(comment['message']),  # Clean the comment message
            emotions, 
            comment['point'], 
            comment['data_utime']
        )
    ...
    def toDict(self):
        return {
            ...
            'replies' : [reply.toDict() for reply in self.replies],
...
def modeBruteID(submode):
    ...
    while (True):
        ...
        if functionData.getStatus() == True:
            with open(f"{udg_storage_dir}/{pageID}.json", 'w', encoding=udg_thaiEncode) as f:
                f.write(crawler.toJson())
            indexFile.seek(0,0)
